...
use crate::{
    code::highlight_code,
    interfaces::{IArticleHeader, InputArticle, InputPublishArticle},
    models::{
...
pub async fn update(
    pool: web::Data<Pool>,
    id: web::Path<i32>,
    article_header: web::Json<IArticleHeader>,
) -> Result<HttpResponse, Error> {
    let connection = pool.get().unwrap();
    Ok(
        web::block(move || Article::update(&connection, &id, article_header.into_inner()))
            .await
            .map(|article| HttpResponse::Ok().json(article))
            .map_err(DatabaseError)?,
    )
...
pub async fn publish(
    ...
    Ok(
        web::block(move || Article::publish(&connection, &id, &published))
            .await
            .map(|article| HttpResponse::Ok().json(article))
            .map_err(DatabaseError)?,
    )
...
pub async fn delete(pool: web::Data<Pool>, id: web::Path<i32>) -> Result<HttpResponse, Error> {
    let connection = pool.get().unwrap();
    Ok(web::block(move || Article::delete(&connection, &id))
        .await
...
pub async fn get(pool: web::Data<Pool>, id: web::Path<i32>) -> Result<HttpResponse, Error> {
    let connection = pool.get().unwrap();
    Ok(web::block(move || Article::find(&connection, &id))
        .await
        .map(|article| HttpResponse::Ok().json(article))
        .map_err(DatabaseError)?)
}
